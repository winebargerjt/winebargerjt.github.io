...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
    if (input$data == 1){
     plot(1:365,temp.mat.Raleigh[,which(years==input$n)], type='p', col="blue", xlab="Time (Days from Jan. 1)", ylab="Avg. Daily Temp (F)", axes = FALSE) 
     axis(side = 2, at = c(0,20,40,60,80,100))
     axis(side = 1, at = c(0,50,100,150,200,250, 300, 350))
     points(1:365, temp.mat.Raleigh[,which(years==input$k)], col="red")
    } else if(input$data == 2){
      plot(1:365,temp.mat.NewOrleans[,which(years==input$n)], type='p', col="blue", xlab="Time (Days from Jan. 1)", ylab="Avg. Daily Temp (F)", axes = FALSE) 
      axis(side = 2, at = c(0,20,40,60,80,100))
      axis(side = 1, at = c(0,50,100,150,200,250, 300, 350))
      points(1:365, temp.mat.NewOrleans[,which(years==input$k)], col="red")
    } else if(input$data == 3){
      plot(1:365,temp.mat.McGuireAFB[,which(years==input$n)], type='p', col="blue", xlab="Time (Days from Jan. 1)", ylab="Avg. Daily Temp (F)", axes = FALSE) 
      axis(side = 2, at = c(0,20,40,60,80,100))
      axis(side = 1, at = c(0,50,100,150,200,250, 300, 350))
      points(1:365, temp.mat.McGuireAFB[,which(years==input$k)], col="red")
    }else{
      plot(1:365,temp.mat.Fairbanks[,which(years==input$n)], type='p', col="blue", xlab="Time (Days from Jan. 1)", ylab="Avg. Daily Temp (F)", axes = FALSE) 
      axis(side = 2, at = c(-60,-40,-20,0,20,40,60,80,100))
      axis(side = 1, at = c(0,50,100,150,200,250, 300, 350))
      points(1:365, temp.mat.Fairbanks[,which(years==input$k)], col="red")
    }
  })
